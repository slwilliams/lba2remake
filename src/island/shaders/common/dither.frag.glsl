...
vec4 dither(float color, float colorIntensity, float intensity) {
    ...
    float fintensity = floor(intensity) + offset + colorIntensity;
    if (fintensity > 14.0 && color < 1.5 && color > 0.5) {
        color += 1.0;
        fintensity = (fintensity - 14.0) + 0.14;
    }
